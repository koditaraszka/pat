...
There is some hardcoding in this method:
  1. the number of simulations performed is set to 1e6, searching for 'sim =' will get the right line if you want to change it
  2. the trait/environmental variance-covariance is assumed to be already standardized 
  3. there are options for using configurations for doing association studies, we found there to be less power when doing so.
You will need to uncomment this in the parser. This has not been thoroughly tested.
'''
...
class Main():
  ...
  def simulate(self):
    ...
    #TODO I'm positive that you could have j go between 0 and math.ceil(self.k/2)
    #I have not checked this but this is not the bottleneck of our code as it's linear and we only allow up to a handful of traits.
  ...
  def map_values(self):
    ...
    lr = self.lrcrit[k]
    mi = self.micrit[k]
    print('pvalue lrcrit micrit')
    print(str(p)+' '+str(lr)+' '+str(mi))
  ...
  def analyze(self):
    ...
    l = Likelihood_Ratio(zs, self.mean, self.null_cov, self.alt_cov, self.model_flag, self.nullprior, self.altprior, self.mvalues)
  ...
  def pvalues(self):
    ...
    mikeep = [int(i-1) for i in mikeep]
...
class Likelihood_Ratio():
  ...
  def __init__(self, sims, mean, null_cov, alt_cov, model, nullprior, altprior, mvalues, log=True):
    ...
    self.results = []
    self.mvalues = []
    self.groups = [[] for i in len(self.mean)]
    if log is False:
  ...
  #set_mvalues returns the posterior marginal for each trait being significant
  def set_mvalues(self,sims):
    include = [list(i) for i in list(itertools.product([1.0, 0.0],repeat=len(self.mean)))]
    for i in include:
      #don't care about the null model 
      if 1 in i:
        x = 
        for j in len(i):
          if j == 1:
