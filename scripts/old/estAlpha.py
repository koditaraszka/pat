...
class Main():
  ...
  def simulate(self):
    ...
    for i in range(0,len(self.env)):
      ...
      sim_v[one][two] = pop_ratio*env_corr*self.imp_var
      sim_v[two][one] = pop_ratio*env_corr*self.imp_var
  ...
  def analyze(self):
    ...
    print('zs[:,0]: ' + str(zs[:,0]))
    #zs = zs[:,self.signif]
    self.alpha = l.set_alpha(zs[self.signif,:], self.sigmaG)
    self.output()
    self.mvalues = l.set_mvalues(zs[self.signif,:], self.alpha)
    self.output2()
  ...
  def output(self):
    ...
    x[self.signif] = self.alpha
    self.combo['Alpha'] = x
    #for i in range(len(self.file)):
      #x = np.zeros(int(self.count))
      #x[self.signif] = self.mvalues[i]
      #self.combo['Mvalue_'+self.file[i][0]] = x
...
class Likelihood_Ratio():
  ...
  def set_alpha(self, sims, sigmaG):
    print('sigmaG:')
    print(sigmaG)
    alpha = []
    maximum = []
    for i in range(100,100000,10):
      covar = self.null_cov + (float(i)/100.0)*sigmaG
      pdf = multivariate_normal.logpdf(sims, self.mean, covar)
      if len(maximum) is 0:
        maximum = pdf
        alpha = np.repeat((float(i)/100.0), len(maximum))
      else:
        combine = np.stack((maximum,pdf), axis=1)
        items = np.argmax(combine,axis=1)
        #print('items length: ' + str(len(items)))
        for j in range(len(items)):
            if items[j] == 1:
              alpha[j] = (float(i)/100.0)
        maximum = np.amax(combine, axis=1)
    print(maximum)
    return alpha

  #set_mvalues returns the posterior marginal for each trait 
  def set_mvalues(self, sims, alpha):
    print('alpha: ' + str(alpha))
    print('sims shape: ' + str(sims.shape))
    mvalues = [[] for i in range(len(self.mean))]
    all_configs = []
    for i in range(0,len(self.alt_configs)):
      value = np.zeros(sims.shape[0])
      for j in range(value.shape[0]):
        print('alpha: ' + str(alpha[j]))
        covar = self.null_cov + alpha[j]*self.alt_configs[i]
        value[j] = multivariate_normal.logpdf(sims[j], self.mean, covar)
      ...
      for j in range(0,len(self.groups)):
        if i in self.groups[j]:
          if len(mvalues[j]) is 0:
            ...
            mvalues[j] = Likelihood_Ratio.sumlog(mvalues[j], value)
            #mvalues[j] = np.add(mvalues[j],value)
    #print(all_configs[:10])
    mvalues = np.exp(np.subtract(mvalues,all_configs))
    #for k in range(0,len(mvalues)):
        #print(mvalues[k][:10])
        #mvalues[k] = np.exp(mvalues[k]-all_configs))
    return mvalues
